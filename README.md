...
**Returns:**
Predecessor of the node with the given id , fingertable of the node with the given id (list of pairs (id, ipaddr:port)).
...
`<registry-ipaddr>:<registry-port>` - ip address anda port number of the Registry.
`<ipaddr>:<port>` - ip address and a port number thisnode will be listening to.
...
Calculates the hash of the given key and the id wherethe text should be stored as follows:
...
**Returns:**
	If successful: (True, Node id it was saved on). Notethat this id may differ from the calculated one.
...
**Returns:**
	...
	If unsuccessful: (False, error message). For example, if the given key was never saved.
